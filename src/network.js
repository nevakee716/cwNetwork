...
(function(cwApi, $) {
  "use strict";
  // constructor
  var network = function(config, diagramTemplate, cwObjects) {
  ...
  network.prototype.addNode = function(object) {
    if (!this.objectTypeNodes.hasOwnProperty(object.group.id)) {
      this.objectTypeNodes[object.group.id] = new cwApi.customLibs.cwLayoutNetwork.objectTypeNode(object.group, object.objectTypeScriptName);
    }
    this.objectTypeNodes[object.group.id].addNode(object,this.diagrammingInformation,this.config);
  };
    ...
    if (elements) {
      elements.forEach(function(elem) {
        if (elem) {
          ...
          if (hasChanged) {
            // on check si le node est pas déja présent dans le réseau
  ...
  network.prototype.getCloseNodes = function(id, edges, option) {
    ...
    edges.forEach(function(edge) {
      if (edge.hidden === false) {
        if (option.ImpactTo && option.ImpactFrom) {
          // on cherche les node qui partent de notre node
          if (edge.from === id) {
        ...
        if (edge.direction) {
          if (option.ImpactTo && tempNode === null) {
            // on cherche les node qui partent de notre node
            if (edge.from === id && edge.direction.indexOf("to") !== -1) {
              tempNode = self.objectTypeNodes[edge.toGroup].getVisDataIfDeactivated(edge.toId, option.highlight);
            } else if (edge.to === id && edge.direction.indexOf("from") !== -1) {
              // si direction both sur le edge
              tempNode = self.objectTypeNodes[edge.fromGroup].getVisDataIfDeactivated(edge.fromId, option.highlight);
            }
          }
          if (option.ImpactFrom && tempNode === null) {
            // on cherche les node qui viennent de notre node
            if (edge.to === id && edge.direction.indexOf("to") !== -1) {
              tempNode = self.objectTypeNodes[edge.fromGroup].getVisDataIfDeactivated(edge.fromId, option.highlight);
            } else if (edge.from === id && edge.direction.indexOf("from") !== -1) {
              // si direction both sur le edge
              tempNode = self.objectTypeNodes[edge.toGroup].getVisDataIfDeactivated(edge.toId, option.highlight);
            }
          }
  ...
  network.prototype.changeState = function(id, groupId, state) {
    this.objectTypeNodes[groupId].changeState(id, state);
  };

  network.prototype.hide = function(id, groupId) {
    this.changeState(id, groupId, false);
  };

  network.prototype.show = function(id, groupId) {
    this.changeState(id, groupId, true);
  };
  ...
  network.prototype.getFilterClusterByGroupHead = function(className) {
    ...
    for (objectType in this.objectTypeNodes) {
      if (this.objectTypeNodes.hasOwnProperty(objectType) && this.objectTypeNodes[objectType].label) {
        object = document.createElement("option");
