  ...
  network.prototype.searchForNodesAndEdges = function (object, nodeOptions) {
    ...
    if (object.hasOwnProperty("children")) {
      for (i = 0; i < object.children.length; i += 1) {
        ...
        if (!nodeOptions.preventEdgeBetweenSameNode || object.children[i].id !== object.id) this.addEdge(object.children[i], object);
    ...
    if (object.name && child.name) {
      ...
      if (!this.edges.hasOwnProperty(uuidAsso) && !this.edges.hasOwnProperty(uuidAssoReverse)) {
        ...
        if (child.direction === "from") {
          this.edges[uuidAssoReverse] = new cwApi.customLibs.cwLayoutNetwork.edge(
            uuid,
            uuidChild,
            object.id,
            child.id,
          ...
          this.edges[uuidAsso] = new cwApi.customLibs.cwLayoutNetwork.edge(
            uuid,
            uuidChild,
            object.id,
            child.id,
