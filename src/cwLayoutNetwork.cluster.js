...
(function(cwApi, $) {
  ...
  cwLayoutNetwork.prototype.clusterByGroup = function() {
    ...
    nodes.forEach(function(node) {
      if ((node.group === self.clusterByGroupOption.head || node.group === self.clusterByGroupOption.head + "_hidden") && !nodeInCluster.hasOwnProperty(node.id)) {
        ...
        if (ncs.length > 0) {
          ...
          ncs.forEach(function(nc) {
            if (heads.indexOf(nc) === -1) {
              if (self.clusterByGroupOption.child.indexOf(self.nodes.get(nc).group.replace("_hidden", "")) !== -1) {
                // if in child group
