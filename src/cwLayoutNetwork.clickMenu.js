...
(function(cwApi, $) {
  ...
  cwLayoutNetwork.prototype.openPopOutFromEdge = function(edge) {
    ...
    if (edge.zipped) {
      let objects = this.network.edges[edge.fromId + "_" + edge.toId].labels;
      if (objects.length === 1) {
        cwApi.cwDiagramPopoutHelper.openDiagramPopout(this.lookForObjects(objects[0].id, objects[0].scriptname, this.copyObject), this.popOut[objects[0].scriptname]);
      } else if (objects.length > 1) {
        cwApi.cwDiagramPopoutHelper.openDiagramPopoutMultipleObjects(objects, this.popOut[objects[0].scriptname]);
      }
      return;
    }
  };
