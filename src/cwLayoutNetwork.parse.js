...
(function(cwApi, $) {
            ...
            if (this.nodeFiltered.hasOwnProperty(associationNode)) {
              ...
              if (!this.objects.hasOwnProperty(element.object_id + "#" + element.objectTypeScriptName + fatherID)) {
                if (this.specificGroup.hasOwnProperty(associationNode)) {
                  ...
                  group = this.specificGroup[associationNode];
                } else {
                  group = cwAPI.mm.getObjectType(nextChild.objectTypeScriptName).name;
                }
                ...
                if (!this.config.groups[group]) {
                  element.group  = this.createRandomGroup(group);
                } else {
                  element.group = this.config.groups[group];
                }
              ...
              element.id = element.object_id + "#" + element.group.id + fatherID;

       

              // add objectType ScriptName to group
              if (element.group && element.objectTypeScriptName && element.group.objectTypes.indexOf(element.objectTypeScriptName) === -1) {
                element.group.objectTypes.push(element.objectTypeScriptName);
              }
  ...
  cwLayoutNetwork.prototype.drawAssociations = function(output, associationTitleText, object) {
    try {
      if (cwApi.customLibs.utils === undefined || cwAPI.customLibs.utils.version === undefined || cwAPI.customLibs.utils.version < 1.2) {
        output.push("<h2> Please Install Utils library 1.2 or higher</h2>");
        return;
      }
